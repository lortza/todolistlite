$( document ).ready(function() {
    ...
    $inputField.on('keyup', function (e) {
      if (e.keyCode == 13) {
        $mainList.prepend('<li class="incomplete"><i class="material-icons check-boxes">check_box_outline_blank</i> '+$(this).val()+' <i class="material-icons delete">highlight_off</i></li>');
        $inputField.val('');
      }
    });
    ...
    $( '#main-list' ).on("click", '.incomplete', function(){
      var $unchecked = $(this).children('i').first().html('check_box_outline_blank');
      $unchecked.html('check_box');
      $(this).addClass('complete').removeClass('incomplete');
    });

    $( '#main-list' ).on("click", '.complete', function(){
      var $checked = $(this).children('i').first().html('check_box');
